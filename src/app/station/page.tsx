    ...
    try {
      ...
      if (stationData.importResults?.imported) {
        toast({
          ...
          description: `Your station is ready with ${
            stationData.importResults.success
          } tracks imported from the playlist${
            stationData.importResults.failed > 0
              ? ` (${stationData.importResults.failed} failed)`
              : ""
          }!`,
          variant: "default",
        });
      } else if (
        stationData.playlistId &&
        (!stationData.importResults || stationData.importResults.success === 0)
      ) {
        toast({
          title: "Station created",
          description:
            "Your station is ready, but we couldn't import tracks from the playlist. You can try importing them later.",
          variant: "default",
        });
      } else {
        toast({
              ...
              <form onSubmit={handleCreateStation} className="space-y-5 mt-5">
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-gray-400 space-y-1">
                    <p>
                      You can find this in the Spotify playlist URL after
                      &quot;playlist/&quot;
                    </p>
                    <p>
                      Adding a playlist ID lets you later import all tracks from
                      that playlist to your station.
                    </p>
                  </div>
                </div>
          ...
          <div className="w-full flex flex-col justify-center items-center h-64 border border-dashed border-gray-700 rounded-lg p-8">
            <div className="text-center space-y-4">
              ...
              <p className="text-gray-400 text-sm">
                Create your first station to start collaborating and listening
                with others.
              </p>
            </div>
            <Button
                        ...
                        <div className="flex flex-row items-center justify-start gap-1 bg-gray-800 p-2 rounded-md mt-1">
                          ...
                          <div className="overflow-hidden">
                            ...
                            <p className="text-sm font-medium truncate text-purple-400">
                              {station.tracks[0].name} -{" "}
                              {station.tracks[0].artist}
                            </p>
                          </div>
                        </div>
                      )}

...
async function fetchChartmastersData(options = { year: "2024", limit: 10000 }) {
  ...
  try {
    console.log(
      `Fetching data from chartmasters.org for year ${
        options.year || "2024"
      }...`
    );
...
if (require.main === module) {
  ...
  console.log(
    `Starting fetch with options: year=${options.year}, limit=${options.limit}`
  );

  fetchChartmastersData(options)
    .then((data) => {
      console.log(
        `Finished processing ${data.length} songs for year ${options.year}`
      );
    })
    .catch((err) => {

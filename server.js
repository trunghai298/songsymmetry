...
app.prepare().then(() => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-station", (stationId, userId, callback) => {
      ...
      console.log(
        `Room station:${stationId} has ${countBefore} members before join`
      );
      ...
      console.log(
        `Room station:${stationId} now has ${countAfter} members after join`
      );
      ...
      console.log(
        `Broadcasting 'user-joined' event to room station:${stationId}`
      );
    ...
    socket.on("leave-station", (stationId, userId, callback) => {
      ...
      const countBefore = roomBefore ? roomBefore.size : 0;
      console.log(
        `Room station:${stationId} has ${countBefore} members before leave`
      );
      ...
      const countAfter = roomAfter ? roomAfter.size : 0;
      console.log(
        `Room station:${stationId} now has ${countAfter} members after leave`
      );

      // Broadcast to ALL clients in the room
      console.log(
        `Broadcasting 'user-left' event to room station:${stationId}`
      );
    ...
    socket.on("add-track", (data) => {
      console.log(
        `User ${data.userId} added track to station ${data.stationId}`
      );
      ...
      console.log(
        `Room station:${data.stationId} has ${memberCount} members when adding track`
      );

      // Notice we're using io.to() for broadcasting to everyone INCLUDING the sender
      console.log(
        `Broadcasting 'track-added' event to all members in room station:${data.stationId}`
      );
    ...
    socket.on("send-message", (data) => {
      console.log(
        `User ${data.userId} sent message to station ${data.stationId}: ${data.message}`
      );
    ...
    socket.on("typing-start", (data) => {
      console.log(
        `User ${data.userId} started typing in station ${data.stationId}`
      );
    ...
    socket.on("typing-stop", (data) => {
      console.log(
        `User ${data.userId} stopped typing in station ${data.stationId}`
      );
